...
- SKElectronics for base sketch 
 ...
 v.3e - FIX blocare pana de curent router (WIFI_STA explicit, timeout conectare WiFi 90s cu restart,
        timeout NTP 30s cu restart, reconectare activa si auto-recovery in loop)
*/
...
// Initialize a NTPClient to get time
  ...
  // Prevenire mod AP nedorit si resetare stiva WiFi
  ...
  while ( WiFi.status() != WL_CONNECTED )
  {
    ...
    // Daca routerul nu raspunde in 90 de secunde (pana curent bloc), dam restart
    if (millis() - startWifi > 90000) {
      Serial.println("\nWiFi timeout la pornire! Restartare...");
      display.fillScreen(display.color565(0, 0, 0));
      TFDrawText (&display, "WiFi ERR", 0, 5, display.color565(nivel, 0, 0));
      delay(2000);
      ESP.restart();
    }
  }
  ...
  {    
    ...
    // Daca routerul s-a conectat dar netul extern nu merge inca, timeout 30s si restart
    if (millis() - startNTP > 30000 && An < 0) {
      Serial.println("\nNTP timeout la pornire! Restartare...");
      display.fillScreen(display.color565(0, 0, 0));
      TFDrawText (&display, "NTP ERR", 0, 5, display.color565(nivel, 0, 0));
      delay(2000);
      ESP.restart();
    }
    delay(1000);
...
void loop() {
  ...
  {
    wifiLostTime = 0; // Resetare cronometru lipsa semnal
  ...
  else  // Daca s-a pierdut conexiunea WiFi in timpul functionarii
  {
    if (wifiLostTime == 0) wifiLostTime = millis();
    display.drawPixel(31, 15, display.color565 (nivel, 0, 0)); // pixel rosu de avertizare

    // Incearca reconectarea la fiecare 10 secunde fara delay blocant
    if (millis() - lastReconnectAttempt > 10000) {
      lastReconnectAttempt = millis();
      Serial.print("WiFi pierdut. Reincerc reconectarea...");
      WiFi.reconnect();
    }

    // Daca sta deconectat mai mult de 3 minute continuu, da restart general
    if (millis() - wifiLostTime > 180000) {
      Serial.println("\nWiFi deconectat prelungit. Restartare ESP...");
      ESP.restart();
    }
  } 
